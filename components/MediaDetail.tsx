...
const MediaDetailHeader: React.FC = () => {
    const { item, loading, onBack } = useMediaDetail();
    if (!item) return null;

    return (
        <PageHeader 
            title="MEDIA DATABASE"
            subtitle={`FILE: ${item.id.toUpperCase()}`}
            icon={Clapperboard}
            status="DECODING..."
...
const MediaDetailHero: React.FC = () => {
    const { item, language } = useMediaDetail();
        ...
        <div className="relative rounded-2xl overflow-hidden bg-slate-900 border border-slate-800 shadow-2xl mb-8 min-h-[400px] flex flex-col md:flex-row">
            ...
            <div className="flex-1 p-6 md:p-10 relative z-10 flex flex-col justify-end md:justify-center">
                ...
                <div className="flex flex-col sm:flex-row gap-6 mt-4 border-t border-slate-800/50 pt-6">
                    <div>
                        <div className="text-[10px] text-slate-500 uppercase tracking-widest font-bold mb-1">Reality Index</div>
...
const MediaDetailAnalysis: React.FC = () => {
    const { analysis, loading } = useMediaDetail();
        ...
        <div className="lg:col-span-2 space-y-6">
            ...
            <Card className="p-6 relative overflow-hidden group">
                ...
                <h3 className="text-sm font-bold text-white mb-4 uppercase tracking-wider flex items-center gap-2">
                    <Search size={16} className="text-accent-cyan" /> The Narrative
                </h3>
                <p className="text-slate-400 text-sm leading-relaxed whitespace-pre-wrap">
                    {analysis?.plotSummary || "Analysis data unavailable."}
                </p>
            </Card>
            ...
            <Card className="p-6 bg-slate-900/80 border-purple-500/20">
                <h3 className="text-sm font-bold text-white mb-4 uppercase tracking-wider flex items-center gap-2">
                    <Eye size={16} className="text-accent-purple" /> Occult Symbolism
                </h3>
            ...
            <Card className="p-6 border-red-500/20 bg-gradient-to-br from-slate-900 to-red-950/10">
                <h3 className="text-sm font-bold text-white mb-4 uppercase tracking-wider flex items-center gap-2">
                    <Zap size={16} className="text-red-500" /> Predictive Programming
                </h3>
...
const MediaDetailConnections: React.FC = () => {
    const { analysis, loading, item, onNavigateToArchive } = useMediaDetail();
    if (!item) return null;
        ...
        <div className="space-y-6 animate-fade-in">
            ...
            <Card className="p-6 bg-slate-950 border-slate-800">
                <h3 className="text-xs font-bold text-slate-500 mb-4 uppercase tracking-wider flex items-center gap-2">
                    <Brain size={14} /> Real World Parallels
                </h3>
            ...
            <Card className="p-6">
                <h3 className="text-xs font-bold text-slate-500 mb-4 uppercase tracking-wider flex items-center gap-2">
                    <ExternalLink size={14} /> Related Archives
                </h3>
